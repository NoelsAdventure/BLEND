...
class CrowdSimPred(CrowdSimVarNum):
    ...
    def generate_ob(self, reset, sort=True):
        ...
        ob = super().generate_ob(reset=reset, sort=True)
    ...
    def step(self, action, update=True):
        ...
        if self.collect_failure_samples:
            ...
            self.store_predictions_to_robot()
        ...
        info={'info':episode_info[0], 'cost': episode_info[1]}
        
        closest_id = None
        closest_dist = 1000
        surrogate_safety_score = 1000
        ...
        for h in self.humans:
            if h.prediction_valid:
                h.gt_locations_aci.append(np.array([h.px, h.py]))
                h.update_aci()
