...
class ShmemVecEnv(VecEnv):
    ...
    def __init__(self, env_fns, spaces=None, context='spawn'):
        ...
        self.parent_pipes = []
        self.procs = []
        self.locks = [ctx.Lock() for _ in env_fns]  # Create a lock for each environment
        with clear_mpi_env_vars():
            for env_fn, obs_buf, lock in zip(env_fns, self.obs_bufs, self.locks):
                wrapped_fn = CloudpickleWrapper(env_fn)
                parent_pipe, child_pipe = ctx.Pipe()
                proc = ctx.Process(target=_subproc_worker,
                            args=(child_pipe, parent_pipe, wrapped_fn, obs_buf, self.obs_shapes, self.obs_dtypes, self.obs_keys, lock))
                proc.daemon = True
    ...
    def update_monitor_async(self, data):
        ...
        observations = [{key: obs[key][index] for key in obs.keys()} for index in range(len(obs['robot_node']))]
        assert len(observations) == len(self.parent_pipes)
        assert len(reward) == len(self.parent_pipes)
        assert len(done) == len(self.parent_pipes)
        assert len(infos) == len(self.parent_pipes)
        for pipe, o, r, d, i in zip(self.parent_pipes, observations, reward, done, infos):
            pipe.send(('update_monitor', (o, r, d, i)))
    ...
    #         pipe.send(('cost2Env', c))
    #     self.waiting_cost2Env = True
    

    # def cost2Env_wait(self):
    #     outs = [pipe.recv() for pipe in self.parent_pipes]  # pipe.recv() is a blocking call
    #     self.waiting_cost2Env = False
...
def _subproc_worker(pipe, parent_pipe, env_fn_wrapper, obs_bufs, obs_shapes, obs_dtypes, keys, lock):
    """
