...
from rl.vec_env.vec_pretext_normalize_dummy import VecPretextNormalizeDummy
...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, config=None, envNum=1, ax=None, test_case=-1):
    def _thunk():
        ...
        elif env_id == "CrowdSimPredRealGST-v0":
            from crowd_sim.envs.crowd_sim_pred_real_gst import CrowdSimPredRealGST
            env = CrowdSimPredRealGST()
        elif env_id == "CrowdSimPredRealGSTRealWorld-v0":
            from crowd_sim.envs.crowd_sim_pred_real_gst_realworld import CrowdSimPredRealGSTRealWorld
            env = CrowdSimPredRealGSTRealWorld()
        else:
...
def make_vec_envs(env_name,
    ...
    if pretext_wrapper: 
        ...
        if gamma is None:
            envs = VecPretextNormalizeDummy(envs, ret=False, ob=False, config=config, test=test)
        else:
            envs = VecPretextNormalizeDummy(envs, gamma=gamma, ob=False, ret=False, config=config, test=test)
