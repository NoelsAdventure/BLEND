...
class PPOLagOriginalUpdate():
    ...
    def update(self, rollouts, mean_ep_costs):
        ...
        for e in range(self.ppo_epoch):
            if self.actor_critic.is_recurrent:
                data_generator = rollouts.recurrent_generator(
                    advantages, torch.zeros_like(advantages), self.num_mini_batch)
            else:
            ...
            for sample in data_generator:
                obs_batch, recurrent_hidden_states_batch, actions_batch, \
                   value_preds_batch, _, return_batch, _, \
                       masks_batch, old_action_log_probs_batch, adv_targ, _ = sample
        ...
        return value_loss_epoch, 0, 0, action_loss_epoch, dist_entropy_epoch, 0, 0